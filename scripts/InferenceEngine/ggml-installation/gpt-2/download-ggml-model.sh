...
mkdir -p ../../../models/ggml/gpt-2/gpt-2-$model

if [ -x "$(command -v wget)" ]; then
    wget --quiet --show-progress -O ../../../models/ggml/gpt-2/gpt-2-$model/ggml-model.bin $src/$pfx-$model.bin
elif [ -x "$(command -v curl)" ]; then
    curl -L --output ../../../models/ggml/gpt-2/gpt-2-$model/ggml-model.bin $src/$pfx-$model.bin
else
...
printf "Done! Model '$model' saved in './models/gpt-2/gpt-2-$model/ggml-model.bin'\n"
printf "You can now use it by setting the API_KEY to:\n\n"
printf "  ./models/ggml/gpt-2-$model/ggml-model.bin\n\n"
